...
export default function AIChatbot() {
    ...
    const handleSend = async (e) => {
        ...
        try {
            ...
            setMessages(prev => [...prev, { role: 'assistant', content: "ERROR: CONNECTION SEVERED. PLEASE RETRY." }]);
        } finally {
            setIsLoading(false);
        }
    };
            ...
            {isOpen && (
                ...
                >
                    ...
                    <div 
                        ...
                        className="flex-1 overflow-y-auto p-5 flex flex-col gap-6 scrollbar-thin scrollbar-thumb-[#ccff00] scrollbar-track-black overscroll-contain"
                    >
                        {messages.map((msg, i) => (
                            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                <div 
                                    className={`
                                        max-w-[85%] p-4 text-xs md:text-sm font-bold leading-relaxed relative
                                        ${msg.role === 'user' 
                    ...
                    <form onSubmit={handleSend} className="p-3 border-t-2 border-[#ccff00] bg-black flex gap-2 shrink-0 z-10">
                        <input 
                            ...
                            className="flex-1 bg-[#111] text-white placeholder:text-gray-600 outline-none text-sm font-bold px-4 py-3 border border-transparent focus:border-[#ccff00] transition-colors"
                        />
                        <button 
                            type="submit" 
