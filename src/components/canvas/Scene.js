...
export default function Home() {
  ...
  useGSAP(() => {
    ...
    // --- WORK SECTION (THE FIX) ---
    // Problem: width was guessed. Solution: Calculate exact scrollable distance.
    const slider = workSliderRef.current;
    
    // We scroll exactly: [Total Width of Slider] - [Width of Viewport]
    function getScrollAmount() {
        let panelsWidth = slider.scrollWidth;
    ...
    const tween = gsap.to(slider, {
        ...
        scrollTrigger: {
            ...
            pin: true,
            scrub: 1,
            // Use a fixed pixel amount for scroll duration (e.g., 4000px of vertical scroll)
            // This decouples the speed from the container width logic.
            end: "+=4000", 
            invalidateOnRefresh: true, // Recalculate on resize
        }
    });

    // --- PROCESS SECTION (STACKING FIX) ---
    // Pin the CONTAINER, not individual cards.
    ...
    // Animate cards 2, 3, 4 over card 1
  ...
  return (
    <main ref={container} onMouseMove={handleMouseMove} className="relative w-full bg-transparent text-black overflow-x-hidden selection:bg-[#ccff00] selection:text-black cursor-none">
      ...
      {/* WORK SECTION (Fixed Width Calculation) */}
      {/* Width: 7 panels x 100vw = 700vw */}
      <section id="work" ref={workRef} className="relative z-30 bg-[#050505] overflow-hidden h-screen">
         <div ref={workSliderRef} className="flex flex-nowrap h-full" style={{ width: '700vw' }}>
            {/* 1. Intro */}
